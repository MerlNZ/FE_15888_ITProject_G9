  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8"> 
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (activity.title === "Kindness Journal") {
                        ...
                        navigate(activity.url); //navogate to Journal
                      }
                    }}
                      >
                    <h3 className="text-2xl font-bold text-blue-500 font-comic">{activity.title}</h3>
                  </div>
                   ))}
                </div>
                </CardContent>
        ...
        {/* Section/column 2 - Student Profile */}           
                            ...
                            <ul className="mt-2 space-y-2 text-left">
                              {upcomingTasks.map((task) => (
                                <li key={task.id} className="flex items-center gap-3 bg-pink-100 p-3 rounded-lg shadow-sm hover:bg-pink-200 transition-all">
                                  <span className="text-red-500 text-xl">📌</span> {/* Pin Icon */}
                                  <span className="font-medium text-lg">{task.title}</span> {/* Task Title */}
                                  <span className="text-gray-600">— Due: {new Date(task.dueDate).toLocaleDateString()}</span> {/* Due Date */}
                                </li>
                              ))}
                            </ul>
                          ) : (
